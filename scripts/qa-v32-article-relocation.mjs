...
const outputDir = "qa-artifacts/article-relocation";
const routes = [
  { key: "ua", path: "/notes/stress-relocation-and-lost-support/", alternate: "/ru/notes/stress-relocation-and-lost-support/", privacy: "/privacy/" },
  { key: "ru", path: "/ru/notes/stress-relocation-and-lost-support/", alternate: "/notes/stress-relocation-and-lost-support/", privacy: "/ru/privacy/" }
];
const viewports = [
...
for (const route of routes) {
  for (const viewport of viewports) {
    ...
    const checks = await page.evaluate(({ alternate, privacy, viewportWidth }) => {
      ...
      const visual = document.querySelector(".article-hero-visual--transition");
      const panel = document.querySelector(".article-transition-panel");
      ...
      return {
        ...
        heroVisible: Boolean(visualRect && visualRect.width > 280 && visualRect.height > 340),
        panelVisible: Boolean(panelRect && panelRect.width > 260 && panelRect.height > 240),
        ...
        sections: ["change", "losses", "anchors", "adaptation", "support-step"].every(id => document.getElementById(id)),
        transitionPoints: document.querySelectorAll(".article-transition-point").length,
        contextColumns: document.querySelectorAll(".article-context-column").length,
        anchorCards: document.querySelectorAll(".article-anchor-card").length,
        rhythmRows: document.querySelectorAll(".article-rhythm-row").length,
        transitionSteps: document.querySelectorAll(".article-transition-step").length,
        supportQuestion: Boolean(document.querySelector(".article-support-question")),
    ...
    const linkedPaths = route.key === "ua"
      ? ["/notes/", "/notes/first-consultation/", "/notes/how-to-start-the-conversation/", "/notes/when-coping-stops-helping/", "/privacy/", "/ru/notes/stress-relocation-and-lost-support/"]
      : ["/ru/notes/", "/ru/notes/first-consultation/", "/ru/notes/how-to-start-the-conversation/", "/ru/notes/when-coping-stops-helping/", "/ru/privacy/", "/notes/stress-relocation-and-lost-support/"];
    const linkedStatus = {};
    ...
    if (!checks.heroVisible) problems.push("transition Hero missing/small");
    if (!checks.panelVisible) problems.push("transition panel missing/small");
    if (!checks.panelInsideHero) problems.push("transition panel outside Hero");
    if (checks.tocCount !== 5) problems.push(`toc=${checks.tocCount}`);
    if (!checks.sections) problems.push("content section missing");
    if (checks.transitionPoints !== 3) problems.push(`transitionPoints=${checks.transitionPoints}`);
    if (checks.contextColumns !== 3) problems.push(`contextColumns=${checks.contextColumns}`);
    if (checks.anchorCards !== 4) problems.push(`anchorCards=${checks.anchorCards}`);
    if (checks.rhythmRows !== 3) problems.push(`rhythmRows=${checks.rhythmRows}`);
    if (checks.transitionSteps !== 3) problems.push(`transitionSteps=${checks.transitionSteps}`);
    if (!checks.supportQuestion) problems.push("support question missing");
