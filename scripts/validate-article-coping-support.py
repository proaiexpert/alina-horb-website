#!/usr/bin/env python3
"""Validate the bilingual observation-oriented V3.2 article."""
...
THEME_CSS = ROOT / "assets/css/site.article.observation.v3-2.css"

PAGES = {
    ROOT / "notes/when-coping-stops-helping/index.html": {
        "lang": "uk",
        "canonical": "https://alinahorb.com/notes/when-coping-stops-helping/",
        "alternate": "https://alinahorb.com/ru/notes/when-coping-stops-helping/",
        "title": "Коли звичні способи справлятися більше не допомагають",
        "privacy": "../../privacy/",
        "diagnosis_boundary": "не визначає діагноз",
    },
    ROOT / "ru/notes/when-coping-stops-helping/index.html": {
        "lang": "ru",
        "canonical": "https://alinahorb.com/ru/notes/when-coping-stops-helping/",
        "alternate": "https://alinahorb.com/notes/when-coping-stops-helping/",
        "title": "Когда привычные способы справляться больше не помогают",
        "privacy": "../../privacy/",
        "diagnosis_boundary": "не определяет диагноз",
    },
}

REQUIRED_IDS = {
    "main-content", "direct-answer-title", "baseline", "dimensions", "observe",
    "support", "contact-step", "author-title", "source-note-title",
    "related-title", "article-cta-title",
}

FORBIDDEN_THRESHOLDS = (
    "через 7 дней", "через 14 дней", "две недели означает", "дві тижні означає",
    "не менее двух недель", "щонайменше два тижні", "три признака", "три ознаки",
    "если есть хотя бы", "якщо є хоча б",
)
...
def validate_page(path: Path, expected: dict[str, str]) -> None:
    ...
    for stylesheet in ("site.article.v3-2.css", "site.article.observation.v3-2.css"):
        if stylesheet not in text:
            fail(f"Missing {stylesheet} in {rel}")
    if "article-theme-observation" not in text:
        fail(f"Observation theme missing in {rel}")
    if expected["diagnosis_boundary"] not in lower:
        fail(f"Diagnosis boundary missing in {rel}")
    if "універсального" not in lower and "универсального" not in lower:
        fail(f"Universal-threshold disclaimer missing in {rel}")
    ...
    if len(re.findall(r'<li><a href="#(?:baseline|dimensions|observe|support|contact-step)"', text)) != 5:
        fail(f"Expected five contents links in {rel}")
    expected_counts = {
        "article-observation-row": 4,
        "article-change-card": 4,
        "article-journal-line": 4,
        "article-support-level": 4,
    ...
    for marker in (
        "article-hero-visual--observation", "article-observation-panel",
        "article-observation-matrix", "article-change-grid", "article-observation-journal",
        "article-support-map", "article-pullquote", "article-message-example",
    ...
    for forbidden in FORBIDDEN_THRESHOLDS:
        if forbidden in lower:
            fail(f"Universal threshold language {forbidden!r} in {rel}")

    if 'https://alinahorb.com/assets/images/notes/alina-horb-note-observation-v3.webp' not in text:
...
def main() -> None:
    ...
    for selector in (
        ".article-hero-visual--observation", ".article-observation-panel",
        ".article-observation-matrix", ".article-change-grid",
        ".article-observation-journal", ".article-support-map",
    ):
        if selector not in theme:
            fail(f"Missing observation selector: {selector}")
    for path, expected in PAGES.items():
        validate_page(path, expected)
    print("V3.2 observation-oriented coping-support article: OK")
